...
public class Program
{
    ...
    public void Solve()
    {
        checked
        {
            ...
            // iより左にある i以下, i以上
            var st = new SegmentTree<int>(MaxA + 1, Math.Max, -1);
            int[] fL = new int[N];
            int[] fG = new int[N];
            for (int i = 0; i < N; i++)
            {
                fL[i] = st.Query(0, A[i] + 1);
                fG[i] = st.Query(A[i], MaxA + 1);

                st[A[i]] = i;
            }

            var st2 = new SegmentTree<int>(MaxA + 1, Math.Min, N);
            int[] bL = new int[N];
            int[] bG = new int[N];
            for (int i = N - 1; i >= 0; i--)
            {
                bL[i] = st2.Query(0, A[i]);
                bG[i] = st2.Query(A[i] + 1, MaxA + 1);
                st2[A[i]] = i;
            }
            ...
            for (int i = 0; i < N; i++)
            {
                ...
                // Console.WriteLine($"A[{i}]={A[i]} {fL[i]} {bL[i]}");
                
                ans += (long) (i - fG[i]) * (bG[i] - i) * A[i];
                ans -= (long) (i - fL[i]) * (bL[i] - i) * A[i];
            }
