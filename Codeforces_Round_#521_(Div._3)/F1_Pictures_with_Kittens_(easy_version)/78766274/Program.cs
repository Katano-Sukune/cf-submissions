...
public class Program
{
    ...
    public void Solve()
    {
        ...
        // iを使った、j個選んだ
        var st = new SegmentTree<long>[X + 1];
        for (int i = 0; i <= X; i++)
        {
            st[i] = new SegmentTree<long>(Math.Max, long.MinValue);
        }
        long ans = -1;
        st[0][0] = 0;
        for (int i = 0; i < N; i++)
        {
            for (int j = 1; j <= Math.Min(X, i + 1); j++)
            {
                // j個選ぶ
                st[j][i + 1] = Math.Max(st[j][i + 1], st[j - 1].Query(Math.Max(0, i - K + 1), i + 1) + A[i]);
                if (j == X && i + K >= N) ans = Math.Max(ans, st[X][i + 1]);
            }
        }
