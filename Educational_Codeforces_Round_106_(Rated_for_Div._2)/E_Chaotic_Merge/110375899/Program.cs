...
public class Program
{
    ...
    public void Solve()
    {
         ...
         *
         * f(l1,r1,l2,r2)
         * X[l1,r1], Y[l2,r2]から Z_{i-1} != Z_iな文字列を作り出せるaの個数
         *
         * 総和
         * 
         */

        // [?,i), [?,j),xから取ったか? y, 最後に取った方
        // 総数
        var dp = new ModInt[n + 1, m + 1, 2, 2, 2];
        for (int i = 0; i <= n; i++)
        {
            for (int j = 0; j <= m; j++)
            {
                for (int k = 0; k <= 1; k++)
                {
                    for (int l = 0; l <= 1; l++)
                    {
                        ...
                        for (int o = 0; o <= 1; o++)
                        {
                            ModInt cur = dp[i, j, k, l, o];
                            if (cur.num == 0) continue;
                            // xから取る
                            if (i < n && X[i] != (o == 0 ? X[i - 1] : Y[j - 1]))
                            {
                                dp[i + 1, j, 1, l, 0] += cur;
                            }

                            if (j < m && Y[j] != (o == 0 ? X[i - 1] : Y[j - 1]))
                            {
                                dp[i, j + 1, k, 1, 1] += cur;
                            }
                        }
