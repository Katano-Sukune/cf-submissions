...
public class Program
{
    ...
    public void Solve()
    {
        checked
        {
            var sc = new Scanner();
            N = sc.NextInt();
            P = new (long X, long Y)[N];
            for (int i = 0; i < N; i++)
            {
                P[i] = (sc.NextLong(), sc.NextLong());
            }

            // Array.Sort(P, (l, r) => l.Y != r.Y ? r.Y.CompareTo(l.Y) : l.X.CompareTo(r.X));

            var map = new Dictionary<long, List<long>>();
            foreach ((long X, long Y) in P)
            {
                if (!map.TryGetValue(Y, out _)) map[Y] = new List<long>();
                map[Y].Add(X);
            }

            var ar = map.ToArray();
            Array.Sort(ar, (l, r) => r.Key.CompareTo(l.Key));
            ...
            foreach ((long _, List<long> ls)in ar)
            {
                // y = _の点を1つ以上使う
                foreach (var x in ls)
                {
                ...
                for (int i = 0; i <= ls.Count; i++)
                {
                    ...
                    long prev = i == 0 ? 0 : set.UpperBound(ls[i - 1]);
                    long cur = i == ls.Count ? set.Count : set.LowerBound(ls[i]);
                    long cnt2 = cur - prev;
                    // Console.WriteLine($"{i} {prev} {cur}");
                    ans -= (cnt2 + 1) * cnt2 / 2;
                }
