...
public class Program
{
    ...
    public void Solve()
    {
        ...
        Array.Sort(tmp, (l, r) => T[l].CompareTo(T[r]));
        ...
        var st1 = new SegmentTree<int>(N, (l, r) => l + r, 0);
        var st2 = new SegmentTree<int>(N, (l, r) => l + r, 0);

        int[] ans = new int[N];
        for (int i = 0; i < N; i++)
        {
            var time = M - T[i];
            int r = st1.MaxRight(0, sum => sum <= time);
            var cnt = st2.Query(0, r);
            ans[i] = i - cnt;
