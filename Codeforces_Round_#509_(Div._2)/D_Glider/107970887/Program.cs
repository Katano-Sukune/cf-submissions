...
public class Program
{
    public void Solve()
    {
        ...
        (int x1, int x2)[] x = new (int x1, int x2)[n];
        for (int i = 0; i < n; i++)
        {
            x[i] = (sc.NextInt(), sc.NextInt());
        }

        Array.Sort(x, (l, r) => l.x1.CompareTo(r.x1));

        var st = new SegmentTree<long>(n, (l, r) => l + r, 0);
        for (int i = 0; i < n; i++)
        {
            st[i] = x[i].x2 - x[i].x1;
        }
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            {
                int mid = (ok + ng) / 2;
                if (h + st.Query(i, mid) > x[mid].x1 - x[i].x1) ok = mid;
                else ng = mid;
            }

            long h2 = h - (x[ok].x1 - x[i].x1 - st.Query(i, ok));
            // Console.WriteLine($"{i} {x[i].x1} {x[ok].x2} {h2} {st.Query(i, ok+1)}");
            ans = Math.Max(ans, x[ok].x2 - x[i].x1 + h2);
        }
