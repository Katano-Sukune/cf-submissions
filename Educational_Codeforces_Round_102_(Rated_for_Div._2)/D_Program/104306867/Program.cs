...
public class Program
{
    ...
    void Q(Scanner sc)
    {
        ...
        var st = new SegmentTree<S>(ar, F, new S(0, 0, 0));
        ...
        for (int i = 0; i < M; i++)
        {
            ...
            var t = F(ff, bb);
            Console.WriteLine(t.Max - t.Min + 1);
            // Console.WriteLine($"{t.Min} {t.Max}");
        }
    }

    S F(S l, S r)
    {
        int min = Math.Min(l.Min, l.Cnt + r.Min);
        int max = Math.Max(l.Max, l.Cnt + r.Max);
        return new S(min, max, l.Cnt + r.Cnt);
    }
...
struct S
{
    public int Min, Max, Cnt;
    public S(int min, int max, int cnt)
    {
        Min = min;
        Max = max;
        Cnt = cnt;
