...
public class Program
{
    ...
    ModInt Go(int cur, int par, HashMap<long, int> map)
    {
        ...
        {
            long gcd = MathEx.GCD(pair.Key, X[cur]);
            next[gcd] += pair.Value;
            ans += (gcd * pair.Value) % ModInt.Mod;
        }
    ...
    public static void Main(string[] args) => new Thread(new ThreadStart(new Program().Solve), 100000000).Start();
}
