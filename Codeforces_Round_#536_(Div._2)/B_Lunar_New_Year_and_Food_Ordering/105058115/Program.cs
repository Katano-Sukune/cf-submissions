...
public class Program
{
    ...
    public void Solve()
    {
        ...
        var set = new Set<int>((l, r) => C[l] != C[r] ? C[l].CompareTo(C[r]) : l.CompareTo(r));
        for (int i = 0; i < N; i++)
        {
            set.Add(i);
        }
        ...
        for (int i = 0; i < M; i++)
        {
            ...
            while (d > 0 && set.Count > 0)
            {
                var v = set[0];
                set.RemoveAt(0);
                int cnt = Math.Min(d, A[v]);
                cost += (long)cnt * C[v];
                d -= cnt;
                A[v] -= cnt;
                if (A[v] > 0) set.Add(v);
