    ...
    public void Solve()
    {
        ...
        for (int i = 0; i < N; i++)
        {
            ...
            for (int d = 1; d <= 10; d++)
            {
                ...
                for (int j = 0; j < len; j++)
                {
                    if (j < Math.Min(lenS, d) * 2)
                    {
                        if (j % 2 == 0) f[i][d] += (str[lenS - 1 - j / 2] - '0') * p;
                        else b[i][d] += (str[lenS - 1 - j / 2] - '0') * p;
                    }
                    else if (lenS > d)
                    {
                        ModInt tmp = p * (str[lenS - 1 - (j - d)] - '0') * p;
                        f[i][d] += tmp;
                        b[i][d] += tmp;
                    }
                    else
