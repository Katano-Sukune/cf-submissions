...
public class Program
{
    ...
    public void Solve()
    {
        ...
        N = sc.NextInt();
        A = sc.IntArray();
        var st = new Stack<(long sum, int len)>();
        for (int i = 0; i < N; i++)
        {
            (long sum, int len) add = (A[i], 1);
            while (st.Count > 0 && Compare(st.Peek(), add) >= 0)
            {
                var tmp = st.Pop();
                add = (add.sum + tmp.sum, add.len + tmp.len);
            }
        ...
        foreach ((long sum, int len) in st)
        {
            for (int i = 0; i < len; i++)
            {
    ...
    int Compare((long sum, int len) l, (long sum, int len) r)
    {
        return (l.sum * r.len).CompareTo(r.sum * l.len);
    }
