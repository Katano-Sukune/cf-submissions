...
public class Program
{
    ...
    public void Solve()
    {
        ...
        for (int i = 0; i < Q; i++)
        {
            ...
            if (t == 0)
            {
                var next = S.UpperBound(x);
                var prev = S.LowerBound(x) - 1;
                if (next < S.Count)
                {
                    D.Remove(S[next] - x);
                }
    ...
    void Query()
    {
        if (S.Count <= 2) Console.WriteLine(0);
        else Console.WriteLine(S[S.Count - 1] - S[0] - D[D.Count - 1]);
    }
