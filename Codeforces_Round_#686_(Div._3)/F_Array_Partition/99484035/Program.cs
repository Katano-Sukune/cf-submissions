...
public class Program
{
    ...
    void Q(Scanner sc)
    {
        ...
        var st = new SegmentTree<int>(a, Math.Min, int.MaxValue);
        var st2 = new SegmentTree<int>(a, Math.Max, int.MinValue);

        for (int x = 1; x <= n - 2; x++)
        {
            int max1 = st2.Query(0, x);
            int ok = x;
            int ng = n + 1;
            ...
            {
                int mid = (ok + ng) / 2;
                int tmpMin2 = st.Query(x, mid);
                int tmpMax3 = st2.Query(mid, n);
                if (tmpMin2 >= max1 && tmpMax3 >= max1) ok = mid;
                else ng = mid;
            }

            int min2 = st.Query(x, ok);
            int max3 = st2.Query(ok, n);

            // if (x == 6)
            // {
            //     Console.WriteLine($"aaa {x} {ok - x} {n - ok}");
            //     Console.WriteLine($"bbb {max1} {min2} {max3}");
            // }

            if (max1 == min2 && min2 == max3)
            {
                Console.WriteLine("YES");
                Console.WriteLine($"{x} {ok - x} {n - ok}");
                return;
            }
        }
