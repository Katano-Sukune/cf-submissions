...
public class Program
{
    ...
    void Q(Scanner sc)
    {
         ...
         * 
         */
        (int sum, int len)[] arF = f.Select(ch => (ch - '0', 1)).ToArray();
        var lst = new LazySegmentTreeACL<(int sum, int len), (int num, bool flag)>(
            arF,
            (l, r) => (l.sum + r.sum, l.len + r.len),
            (0, 0),
            (f, x) => f.flag ? (x.len * f.num, x.len) : x,
            (f, g) => f.flag ? f : g,
            (0, false)
            );
        ...
        for (int i = q - 1; i >= 0; i--)
        {
            var tuple = lst.Query(l[i], r[i]);
            if (tuple.sum < (r[i] - l[i] + 1) / 2)
            {
                // 0に揃える
                lst.Apply(l[i], r[i], (0, true));
            }
            else if ((r[i] - l[i] - tuple.sum) < (r[i] - l[i] + 1) / 2)
            {
                // 1に揃える
                lst.Apply(l[i], r[i], (1, true));
            }
            else
            {
