...
public class Program
{
    ...
    public void Solve()
    {
        ...
        // 持ってるX
        var stX = new Set<long>();
        // 持ってるY
        var stY = new Set<long>();
        ...
        var st = new SegmentTree<long>(K, (l, r) => l + r, 0);
        ...
        for (int i = 0; i < N; i++)
        {
            ...
            {
                // Y最小だけ倍にできない
                Console.WriteLine(2 * st.Query(0, K) - stY[0]);
            }
            else if (stY.Count == 0)
            {
                Console.WriteLine(st.Query(0, K));
            }
            else if (stX.Count > 0 && stY.Count > 0 && stY[0] > stX[stX.Count - 1])
            {

                // 合計
                long sum = st.Query(0, K);

                // 上位k個合計
                var t = set[set.Count - stY.Count];
                long sum2 = st.Query(map[(int)t], K);
                Console.WriteLine(sum + sum2 - stY[0] + stX[stX.Count - 1]);
            } 
            else
            {
                // 合計
