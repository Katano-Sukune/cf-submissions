...
public class Program
{
    ...
    public void Solve()
    {
        ...
        // テレビを消しても 少なくとも1つ点いてる時間が変わらないテレビ
        // 無いなら-1

        (int idx, int time, bool end)[] A = new (int idx, int time, bool end)[2 * N];
        for (int i = 0; i < N; i++)
        {
            A[i] = (i, L[i], false);
            A[i + N] = (i, R[i] + 1, true);
        ...
        Array.Sort(A, (l, r) => l.time.CompareTo(r.time));
        ...
        while (ptr < 2 * N)
        {
            if (ptr < 2 * N) time = A[ptr].time;
            while (ptr < 2 * N && A[ptr].time <= time)
            {
                if (A[ptr].end)
                {
                    set.Remove(A[ptr].idx);
                }
                else
                {
                    set.Add(A[ptr].idx);
            ...
            if (set.Count == 1)
            {
                f[set.ToArray()[0]] = true;
            }
        }
        int ans = Array.IndexOf(f, false);
        if (ans != -1) ans++;
