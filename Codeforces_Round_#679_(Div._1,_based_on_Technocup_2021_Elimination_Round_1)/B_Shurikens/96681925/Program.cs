...
public class Program
{
    ...
    public void Solve()
    {
        ...
        var st = new SegmentTree<int>(N + 1, Math.Max, int.MinValue);
        for (int i = 0; i < 2 * N; i++)
        {
            if (T[i] == '+') cnt++;
            else
            {
                r[X[i]] = cnt;
                var q = st.Query(X[i] + 1, N + 1);
                if (q == int.MinValue) l[X[i]] = 0;
                else l[X[i]] = q;
                st[X[i]] = cnt;
            }
        }

        var add = new List<int>[N];
        for (int i = 0; i < N; i++)
        {
            add[i] = new List<int>();
        }
        ...
        var pq = new PriorityQueue<int>((x, y) => r[x].CompareTo(r[y]));
        int[] ans = new int[N];
        for (int i = 0; i < N; i++)
        {
            foreach (int j in add[i])
            {
                pq.Enqueue(j);
            }

            if (pq.Count == 0 || r[pq.Peek()] <= i)
            {
                Console.WriteLine("NO");
                return;
            }
