...
public class Program
{
    ...
    public void Solve()
    {
        ...
        var set = new Set<int>((l, r) => index[l].CompareTo(index[r]));
        ...
        {
            int max = set2[set2.Count - 1];
            int ind = set.LowerBound(max);
            int l = Math.Max(0, ind - K);
            int r = Math.Min(set.Count, ind + K + 1);
            for (int j = 0; j < r - l; j++)
            {
                int num = set[l];
                ans[index[num]] = i % 2 == 0 ? '1' : '2';
                set.Remove(num);
                set2.Remove(num);
            }
        }
