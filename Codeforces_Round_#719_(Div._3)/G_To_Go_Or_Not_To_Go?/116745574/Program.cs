...
public class Program
{
    ...
    public void Solve()
    {
        ...
        var distS = BFS(0, 0);
        var distG = BFS(N - 1, M - 1);

        long ans = long.MaxValue;

        // ワープしない
        if (distS[N - 1][M - 1] != int.MaxValue) ans = Math.Min(ans, (long)distS[N - 1][M - 1] * W);
        long minS = long.MaxValue;
        long minG = long.MaxValue;
        for (int i = 0; i < N; i++)
        {
            for (int j = 0; j < M; j++)
            {
                if (A[i][j] <= 0) continue;
                if (distS[i][j] != int.MaxValue) minS = Math.Min(minS, (long)distS[i][j] * W + A[i][j]);
                if (distG[i][j] != int.MaxValue) minG = Math.Min(minG, (long)distG[i][j] * W + A[i][j]);
            }
        }

        if (minS != long.MaxValue && minG != long.MaxValue) ans = Math.Min(ans, minS + minG);
    ...
    int[][] BFS(int r, int c)
    {
        ...
        while (q.Count > 0)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                if (A[nr][nc] == -1 || ans[nr][nc] <= ans[rr][cc] + 1) continue;
