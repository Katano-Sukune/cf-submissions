...
public class Program
{
    ...
    public void Solve()
    {
        ...
        // やってるやつ
        var process = new List<(int a, int b, int i)>();
        // 解決済み
        int cnt = 0;

        int ptr = 0;
        while (process.Count < K && ptr < N)
        {
            process.Add((A[ptr], 0, ptr));
            ptr++;
        }
        ...
        {
            int nCnt = cnt;
            var next = new List<(int a, int b, int i)>();
            foreach (var t in process)
            {
                if (t.b + 1 == (1000 * cnt + 5 * N) / (10 * N))
                {
                    f[t.i] = true;
                }
                if (t.b + 1 < t.a)
                {
                    next.Add((t.a, t.b + 1, t.i));
                }
                else
                {
                    nCnt++;
            ...
            while (next.Count < K && ptr < N)
            {
                next.Add((A[ptr], 0, ptr));
                ptr++;
            }
            cnt = nCnt;
            process = next;
