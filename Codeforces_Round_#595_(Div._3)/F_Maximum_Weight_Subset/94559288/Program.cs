...
public class Program
{
    ...
    public void Solve()
    {
        checked
        {
            ...
            E.Build();
            // 0根
            // dp[i,j] 部分木i、一番浅い頂点までの距離, スコア最大
            DP = new SegmentTree<long>[N];
            for (int i = 0; i < N; i++)
            {
                DP[i] = new SegmentTree<long>(K + 1, Math.Max, int.MinValue);
            }

            Go(0, -1);
            Console.WriteLine(DP[0].All());
        }
    }
    ...
    void Go(int cur, int par)
    {
        ...
        for (int i = 0; i < E[cur].Length; i++)
        {
            ...
            {
                if (DP[vi][di] == int.MinValue) continue;
                long score = DP[vi].Query(di, K + 1);
                for (int j = 0; j < E[cur].Length; j++)
                {
                    // その他
                    if (i == j) continue;
                    int vj = E[cur][j];
                    ...
                    // di + 1 +  dj + 1 >= k+1
                    // di + dj + 1 >= k
                    score += DP[vj].Query(Math.Max(di, K - di - 1), K + 1);
                }

                if (DP[cur][di + 1] < score)
                {
                    DP[cur][di + 1] = score;
                }
            }
