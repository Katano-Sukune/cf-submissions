...
namespace CompLib.Collections
{
    ...
    public class WaveletMatrix
    {
        ...
        /// <summary>
        /// [begin, end)でk個以上ある数のリスト
        /// </summary>
        /// <param name="l"></param>
        /// <param name="r"></param>
        /// <param name="k"></param>
        /// <returns></returns>
        public List<PairNumFreq> WhereCountK(int begin, int end, int k)
        {
            var q = new Queue<Segment>();
            q.Enqueue(new Segment(begin, end, D - 1, 0));
            var result = new List<PairNumFreq>();
            while (q.Count > 0)
            {
                var dq = q.Dequeue();
                int b = dq.Begin;
                ...
                if (d < 0)
                {
                    result.Add(new PairNumFreq(n, e - b));
                    continue;
                }
                ...
                if (r0 - l0 >= k)
                {
                    ...
                    q.Enqueue(new Segment(l0, r0, d - 1, n));
                }
                ...
                if (r1 - l1 >= k)
                {
                    q.Enqueue(new Segment(l1, r1, d - 1, n | ((Num)1 << d)));
                }
            }
            return result;
        }
