...
public class Program
{
    ...
    void Q(Scanner sc)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            int p = set.LowerBound(a[i]);
            if (p < set.Count && set[p] == a[i]) flag[i] = 1;
        }
        ...
        if (t < n)
        {
            ...
            for (int i = t; i < n; i++)
            {
                int len = i - t + 1;
                int min = a[i] - (i - t);
                int max = i == n - 1 ? int.MaxValue : a[i + 1] - (i - t + 1);

                int p = set.LowerBound(min);
                int q = set.LowerBound(max);
                for (int j = p; j < q; j++)
                {
                    // tをset[j]まで押す
                    int r = set.LowerBound(set[j] + len);
                    tmp = Math.Max(tmp, r - j + sum[n] - sum[i + 1]);
                }
            }
        ...
        if (t > 0)
        {
            ...
            for (int i = t - 1; i >= 0; i--)
            {
                ...
                int len = t - i;
                int max = a[i];
                int min = i == 0 ? int.MinValue : a[i - 1];
                int p = set.UpperBound(min);
                int q = set.UpperBound(max);
                for (int j = p; j < q; j++)
                {
                    // 
                    int r = set.LowerBound(set[j] + len);
                    tmp = Math.Max(tmp, r - j + sum[i]);
                }
            }
