...
public class Program
{
    ...
    private SegmentTree<int> StMin, StMax;
    private SegmentTree<long> StSum;
    ...
    void Query(Scanner sc)
    {
        ...
        // mid = floor((max(a) + min(a)) / 2)

        // mid以下のleft, mid超過のrightに分割
        // すきな方を選ぶ
        // 繰り返し
        // 総和がs_iにできるか?

        Array.Sort(A);
        StMin = new SegmentTree<int>(A, Math.Min, int.MaxValue);
        StMax = new SegmentTree<int>(A, Math.Max, int.MinValue);
        StSum = new SegmentTree<long>(A.Select(num => (long) num).ToArray(), (l, r) => l + r, 0);
        ...
        for (int i = 0; i < Q; i++)
        {
            Console.WriteLine(Hs.Contains(S[i]) ? "Yes" : "No");
        }
    }
    ...
    {
        if (l == r) return;
        long sum = StSum.Query(l, r);
        if (sum <= (long) 1e9) Hs.Add(sum);
