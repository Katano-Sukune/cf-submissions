...
public class Program
{
    ...
    void Q(Scanner sc)
    {
        ...
        (long l, long r) = Go(0, -1);
        Console.WriteLine(Math.Max(l, r));
    }

    (long l, long r) Go(int cur, int par)
    {
        long l = 0;
        long r = 0;
        ...
        {
            if (to == par) continue;
            (long tl, long tr) = Go(to, cur);
            l += Math.Max(Math.Abs(L[cur] - L[to]) + tl, Math.Abs(L[cur] - R[to]) + tr);
            r += Math.Max(Math.Abs(R[cur] - L[to]) + tl, Math.Abs(R[cur] - R[to]) + tr);
        }
        return (l, r);
    }
