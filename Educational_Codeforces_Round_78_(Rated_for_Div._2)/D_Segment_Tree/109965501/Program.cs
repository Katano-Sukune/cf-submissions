...
public class Program
{
    ...
    public void Solve()
    {
        ...
        // rの位置
        var set = new Set<(int v, int r)>((l, r) => l.r.CompareTo(r.r));
        ...
        Array.Sort(sorted, (l, r) => L[l].CompareTo(L[r]));

        foreach (int i in sorted)
        {
            // l_i以上 r_i未満にあるr
            int p = set.LowerBound((-1, L[i]));
            for (; p < set.Count; p++)
            {
                var pair = set[p];
                if (pair.r >= R[i]) break;
                
                // Console.WriteLine($"{i} {pair.v} {pair.r}");
                if (!uf.Connect(i, pair.v))
                {
        ...
        Console.WriteLine(uf.GetSize(0) == N ? "YES" : "NO");
    }
