...
public class Program
{
    ...
    public void Solve()
    {
        ...
        M = sc.NextInt();
        K = sc.NextInt();
        Dictionary<Path, long> map = new Dictionary<Path, long>();
        Set<Path> set = new Set<Path>((l, r) => map[l].CompareTo(map[r]));
        set.IsMultiSet = true;
        ...
        Array.Sort(index, (l, r) => w[l].CompareTo(w[r]));

        // パスに使われる可能性がある辺は長さ下位K位

        for (int i = 0; i < Math.Min(K, M); i++)
        {
            var p = new Path(u[index[i]], v[index[i]]);
            map[p] = w[index[i]];
            set.Add(p);
            edges[u[index[i]]].Add(new Edge(v[index[i]], w[index[i]]));
            edges[v[index[i]]].Add(new Edge(u[index[i]], w[index[i]]));
        }
        ...
        for (int i = 0; i < K; i++)
        {
            var p = set[i];
            long l = map[p];
            foreach (var e in edges[p.F])
            {
                if (p.T == e.To) continue;
                var np = new Path(p.T, e.To);
                long o;
                if (map.TryGetValue(np, out o))
                {
                    if (l + e.W < o)
                    {
                        set.Remove(np);
            ...
            foreach (var e in edges[p.T])
            {
                if (p.F == e.To) continue;
                var np = new Path(p.F, e.To);
                long o;
                if (map.TryGetValue(np, out o))
                {
                    if (l + e.W < o)
