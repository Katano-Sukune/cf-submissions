...
public class Program
{
    ...
    public void Solve()
    {
        ...
        // 長さK 数列x
        // x_i xor x_{i+1} が3の倍数

        // 何通りできるか?
        // 次にして良いやつ

        // iで始まってjで終わるパターン
        Array2D ans = new Array2D(N, N);

        // 2^n回で iで始まってjで終わるパターン
        Array2D t = new Array2D(N, N);
        for (int i = 0; i < N; i++)
        {
            for (int j = 0; j < N; j++)
            {
                t[i, j] = Algorithm.BitCount(A[i] ^ A[j]) % 3 == 0 ? 1 : 0;
                ans[i, j] = i == j ? 1 : 0;
            }
        }
...
#if DEBUG
        Console.WriteLine(ModInt.Pow(N, K));
#endif
    }

    Array2D Mul(Array2D l, Array2D r)
    {
        var res = new Array2D(N, N);
        for (int i = 0; i < N; i++)
        {
            for (int j = 0; j < N; j++)
            {
