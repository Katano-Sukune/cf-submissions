...
public class Program
{
    ...
    public void Solve()
    {
        ...
        {
            int t = sc.NextInt() - 1;
            if (set[t].Count > 0 && set[t][set[t].Count - 1] == i - 1)
            {
                ans--;
            }
        ...
        for (int i = 0; i < M - 1; i++)
        {
            ...
            foreach (int j in ar)
            {
                int itr = large.UpperBound(j);
                if (itr < large.Count && large[itr] == j + 1) ans--;
                if (itr - 1 >= 0 && large[itr - 1] == j - 1) ans--;
            }
            foreach (int j in ar)
            {
                large.Add(j);
