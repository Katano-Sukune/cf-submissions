...
public class Program
{
    ...
    {
        Ans = new long[Max + 1];
        var map = new long[2 * Max + 1];
        var primes = new List<int>();
        MathEx.Sieve(30000, primes);
        ...
        for (int i = 0; i < primes.Count; i++)
        {
            ...
            {
                int d = Math.Min(primes[i] - 1, primes[j] - primes[i]);
                if (d <= 2 * Max)
                {
                    long num = (long)primes[i] * primes[j];

                    map[d] = Math.Min(map[d], num);
                }
            }
            int d2 = primes[i] - 1;
            ...
            if (d2 <= 2 * Max)
            {
                long num2 = (long)primes[i] * primes[i] * primes[i];
                map[d2] = Math.Min(map[d2], num2);
            }
        }
