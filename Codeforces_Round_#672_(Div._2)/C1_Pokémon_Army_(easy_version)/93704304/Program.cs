...
public class Program
{
    ...
    void Q(Scanner sc)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            ar[i] = new S(a[i], long.MinValue, long.MinValue, -a[i]);
        }

        var st = new SegmentTree<S>(ar,
            (l, r) =>
            {
                S result = new S(long.MinValue, long.MinValue, long.MinValue, long.MinValue);
                for (int i = 0; i < 4; i++)
                {
                    if (l[i] == long.MinValue) continue;
                    result[i] = Math.Max(result[i], l[i]);
                    for (int j = 0; j < 4; j++)
                    {
                        if (r[j] == long.MinValue) continue;
                        result[j] = Math.Max(result[j], r[j]);
                        if (i / 2 == j % 2) continue;
                        int k = i % 2 + (j / 2) * 2;
                        result[k] = Math.Max(result[k], l[i] + r[j]);
                    }
                }

                return result;
            },
            new S(long.MinValue, long.MinValue, long.MinValue, long.MinValue));
        var all = st.All();
        // for (int i = 0; i < n; i++)
        // {
        //     var tmp = st.Query(0, i + 1);
        //     Console.Write(tmp[0] + ":" + tmp[2] + " ");
        // }
        //
        // Console.WriteLine();
        Console.WriteLine(Math.Max(all[0], all[2]));
        for (int i = 0; i < q; i++)
        {
            (st[l[i]], st[r[i]]) = (st[r[i]], st[l[i]]);
            all = st.All();
            Console.WriteLine(Math.Max(all[0], all[2]));
        }
    }
...
struct S
{
    private long Zero, One, Two, Three;

    public S(long zero, long one, long two, long three)
    {
        Zero = zero;
        One = one;
        Two = two;
