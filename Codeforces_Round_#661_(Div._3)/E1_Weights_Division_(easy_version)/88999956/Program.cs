...
public class Program
{
    ...
    private int N;
    private long S;
    private List<(int to, long w)>[] E;
    private PriorityQueue<(long w, int cnt)> Pq;
    private long Sum;
    ...
    void Q(Scanner sc)
    {
        ...
        N = sc.NextInt();
        S = sc.NextLong();
        E = new List<(int to, long w)>[N];
        for (int i = 0; i < N; i++)
        {
            E[i] = new List<(int to, long w)>();
        }
        ...
        // 重さw 下にcnt個頂点
        Pq = new PriorityQueue<(long w, int cnt)>((l, r) =>
        {
        ...
        while (Sum > S)
        {
            ...
            Sum -= m;
            Pq.Enqueue((d.w / 2, d.cnt));

            // Console.WriteLine($"{d.w} {d.cnt} {Sum}");
            ans++;
        }
