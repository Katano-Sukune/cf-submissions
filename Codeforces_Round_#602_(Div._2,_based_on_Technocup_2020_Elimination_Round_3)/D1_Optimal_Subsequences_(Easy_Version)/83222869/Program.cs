...
public class Program
{
    ...
    public void Solve()
    {
        ...
        Array.Sort(sorted, (l, r) =>
        {
            ...
            {
                return A[r].CompareTo(A[l]);
            }
        });

        var sortedQ = new int[M];
        for (int i = 0; i < M; i++)
        {
            sortedQ[i] = i;
        }

        Array.Sort(sortedQ, (l, r) => K[l].CompareTo(K[r]));
        ...
        foreach (int i in sortedQ)
        {
            while (set.Count < K[i])
            {
                set.Add(sorted[index++]);
            }
