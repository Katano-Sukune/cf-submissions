...
public class Program
{
    ...
    public void Solve()
    {
        ...
        List<(int pos, int j)>[] query = new List<(int pos, int j)>[N + 1];
        for (int i = 0; i <= N; i++)
        {
            query[i] = new List<(int pos, int j)>();
        }

        for (int j = 0; j < M; j++)
        {
            query[K[j]].Add((Pos[j], j));
        }
        ...
        Array.Sort(index, (l, r) => A[l] == A[r] ? l.CompareTo(r) : A[r].CompareTo(A[l]));
        ...
        {
            s.Add(index[i - 1]);
            foreach ((int pos, int j) in query[i])
            {
                ans[j] = A[s[pos - 1]];
            }
        }
