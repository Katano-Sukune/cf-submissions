...
public class Program
{
    ...
    public void Solve()
    {
        ...
        for (int i = 0; i < t; i++)
        {
            Console.WriteLine(Q(sc.NextInt(), sc.NextInt(), sc.IntArray(), sc.IntArray()));
        }
        Console.Out.Flush();
    }

    string Q(int n, int k, int[] a, int[] b)
    {
        // k回まで 
        var pqA = new PriorityQueue<int>((l, r) => l.CompareTo(r));
        var pqB = new PriorityQueue<int>((l, r) => r.CompareTo(l));
        ...
        for (int i = 0; i < k && pqA.Peek() < pqB.Peek(); i++)
        {
            var dA = pqA.Dequeue();
            var dB = pqB.Dequeue();
            pqA.Enqueue(dB);
