...
public class Program
{
    ...
    string Q(int n)
    {
        var pq = new PriorityQueue<Segment>((l, r) => (l.R - l.L) != (r.R - r.L) ? (r.R - r.L).CompareTo(l.R - l.L) : l.L.CompareTo(r.L));
        pq.Enqueue(new Segment(0, n));
        var ans = new int[n];
        for (int i = 1; i <= n; i++)
        {
            var d = pq.Dequeue();
            int index = d.L + (d.R - d.L - 1) / 2;
            ans[index] = i;
            if (index - d.L > 0) pq.Enqueue(new Segment(d.L, index));
            if (d.R - index - 1 > 0) pq.Enqueue(new Segment(index + 1, d.R));
        }
