...
public class Program
{
    ...
    public void Solve()
    {
        ...
        N = sc.NextInt();
        A = sc.LongArray();
        St = new SegmentTree<long>(N, (Math.Min), long.MaxValue);
        for (int i = 0; i < N; i++)
        {
            St[i] = A[i];
        }
        ...
        if (f) ans += Calc(left, N, 0);
        Console.WriteLine(ans);
    }

    long Calc(int l, int r, long t)
    {
        long a = r - l;

        long min = St.Query(l, r);

       //  Console.WriteLine($"{l} {r} {min}");
        long b = min - t;
        bool f = false;
        int left = -1;
        for (int i = l; i < r; i++)
