...
public class Program
{
    ...
    public void Solve()
    {
        ...
        for (int i = 0; i < M; i++)
        {
            ...
            for (int j = 0; j < K; j++)
            {
                sum += Math.Min(d[A[j], B[j]], Math.Min(d[A[j], X[i]] + d[Y[i], B[j]], d[A[j], Y[i]] + d[X[i], B[j]]));
            }
    ...
    void F(int b)
    {
        ...
        while (pq.Count > 0)
        {
            (int node, long dist) = pq.Dequeue();
            if (ar[node] < dist) continue;
            foreach ((int to, int w) in e[node])
            {
                if (dist + w < ar[to])
                {
                    ar[to] = dist + w;
