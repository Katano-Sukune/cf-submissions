...
public class Program
{
    ...
    public void Solve()
    {
        ...
        // st[j][i] i行j列から左に同色　個数
        var st = new SegmentTree<int>[M];
        for (int j = 0; j < M; j++)
        {
            st[j] = new SegmentTree<int>(N, Math.Min, int.MaxValue);
        }

        // cnt[i,j] i行j列から下に同色　個数
        int[,] cnt = new int[N, M];
        ...
        for (int i = 0; i < N; i++)
        {
            for (int j = 0; j < M; j++)
            {
                ...
                // a個下にもa個並んでいるか?
                if (cnt[i + a, j] != a) continue;
                // そのまたa個下にもa個以上ならんでいるか?
                if (cnt[i + 2 * a, j] < a) continue;
                
                // 左にいくつか
                int l = st[j].Query(i, i + 3 * a);
                ans += l;
            }
        }
