...
public class Program
{
    ...
    public void Solve()
    {
        ...
        Array.Sort(idx, (l, r) => K[l].CompareTo(K[r]));
        ...
        foreach (int i in idx)
        {
            long l = K[i] - N - 1;
            // 1ループさせる
            while (ph <= max && t + set.Count <= l)
            {
                t += set.Count;
                foreach (int j in ls[ph])
                {
                    set.Add(j);
                }
                ph++;
            ...
            // Console.WriteLine($"{i} {l} {t} {set.Count}");
            ans[i] = set[(int)((l - t) % set.Count)] + 1;
