...
public class Program
{
    ...
    public void Solve()
    {
        ...
        var map = new Dictionary<long, List<(int u, int v)>>();
        for (int i = 0; i < M; i++)
        {
            int u = sc.NextInt() - 1;
            int v = sc.NextInt() - 1;
            ...
            long xor = C[u] ^ C[v];
            if (!map.TryGetValue(xor, out var _)) map[xor] = new List<(int u, int v)>();
            map[xor].Add((u, v));
        }
        ...
        {
            var ls = new List<int>();
            foreach ((int u, int v) in pair.Value)
            {
                ls.Add(u);
                ls.Add(v);
            }
            ...
            var uf = new UnionFind(p);
            int cnt2 = N;
            foreach ((int u, int v) in pair.Value)
            {
                if (uf.Connect(map2[u], map2[v])) cnt2--;
            }

            ans += ModInt.Pow(2, cnt2);
        }

        ans += ModInt.Pow(2, N) * ((1L << K) - map.Count);
